<?php
    ...
    $qryreg = "SELECT `reg`.`schlenrollprereg_regdate` `REG_DATE`,
        ...
        UPPER(`crse`.`schlacadcrse_code`) `CRSE_NAME`,
            `reg`.`schlenrollprereg_id` `STUD_ID`, 
        ...
        <div id="prereg-data"></div>
            <table id='regtable' class='table caption-top table-hover'>
                ...
                <?php 
                    ...
                    if ($rsreg -> num_rows >  0) 
                    {
                        while ($fetchDatareg = $rsreg->fetch_assoc()) 
                        {
                            $count=$count+1;
                ?>
                    <tr>
                        <td><?php echo $fetchDatareg["REG_DATE"]        ?></td>
                        <td><?php echo $fetchDatareg["NAME"]            ?></td>
                        <td><?php echo $fetchDatareg["LVL_NAME"]        ?></td>
                        <td><?php echo $fetchDatareg["YRLVL_NAME"]      ?></td>
                        <td><?php echo $fetchDatareg["CRSE_NAME"]       ?></td>
                        <td>
                            <select id='user_list' name='user_list' class='form-control dropdown'>
                                <?php
                                    $rsuser = $dbConn->query($qryuser);
                                    $count=0;
                        ...
                        <td>
                            ...
                            <a href='enrollmentregistrationform.php?userid="<?php echo $rsuser_row["VIEW"]?>"' target = '_blank'>
                            	<button type='button' class='btn btn-primary'>View Profile</button></a>
                            <input type='hidden' id='".$regitem['VIEW']."' name='prereg_id' value='".$regitem['VIEW']."'>

                            <button type='button' class='btn btn-danger' hidden>Process</button>
                            <input type='hidden' id='".$regitem['PROCESS']."' name='prereg_id' value='".$regitem['PROCESS']."'>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
            $("#user_list").change(function(){
                $("#loader").show();
                var get_user_list_Id = $(this).val();
                var get_stud_list_Id = $(<?php echo $fetchDatareg["STUD_ID"];?>).val();

                $.ajax({
                    type:'POST',
                    url: 'test(php).php',
                    data: { user_id:get_user_list_Id,
                            stud_id:et_stud_list_Id
                          }
                });
            });
